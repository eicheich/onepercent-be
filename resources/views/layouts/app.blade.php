        ...
        <aside
            ...
            <nav class="flex-1 p-4 space-y-2 overflow-y-auto">
                ...
                <a href="{{ route('web.friends') }}"
                    ...
                    <img class="w-5 h-5" src="/img/ic_laugh.png" />
                    <span>Friends</span>
                </a>

                <a href="{{ route('web.notifications') }}"
                    class="sidebar-link flex items-center gap-3 px-4 py-3.5 rounded-[1.25rem] {{ request()->routeIs('web.notifications') ? 'active' : '' }}">
            ...
            <div class="p-4 m-4 mt-0 bg-background/50 rounded-[1.5rem] border border-white">
                ...
                <form method="POST" action="{{ route('web.logout') }}">
                    @csrf
                    <button
                        class="w-full text-center px-3 py-2.5 text-sm font-bold text-red-500 hover:bg-red-500 hover:text-white rounded-[1rem] transition-all flex items-center justify-center gap-2 group">
            ...
            <header
                ...
                <div class="flex items-center gap-4">
                    ...
                    <a href="{{ route('web.notifications') }}"
                        ...
                        @php
                            $unread = \App\Models\ChallengePoke::where('receiver_id', session('web_user.id'))
                                ->whereNull('read_at')
                                ->count();
                        @endphp
