...
@section('content')
    <div class="space-y-6 md:space-y-8 fade-in pb-10">
        ...
        {{-- Search bar --}}
        <div class="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#EAE1FF] p-4 md:p-6 relative overflow-hidden group">
            {{-- Aksen Blob Background --}}
            <div class="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-[#F3EFFF] to-transparent rounded-full -mr-10 -mt-10 opacity-50 pointer-events-none"></div>

            <form method="GET" action="{{ route('web.friends.search') }}" class="flex flex-col md:flex-row gap-3 relative z-10">
        ...
        @if (isset($query) && $query)
            <div class="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#EAE1FF] overflow-hidden">
                    ...
                    <div class="divide-y divide-slate-50">
                        @foreach ($users as $u)
                            <div class="flex items-center gap-4 px-6 py-4 hover:bg-[#F3EFFF]/30 transition-colors">
                                {{-- Avatar --}}
                                <div class="w-12 h-12 rounded-[1rem] flex items-center justify-center font-black text-base flex-shrink-0 shadow-inner
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="font-black text-slate-800 text-base truncate tracking-tight">{{ $u->name }}</p>
                                    <p class="text-sm text-slate-400 truncate font-medium">{{ $u->email }}</p>
                                </div>

                                {{-- Streak --}}
                                <div class="hidden md:flex items-center gap-1.5 shrink-0 bg-white border border-slate-100 px-3 py-1.5 rounded-full shadow-sm">
                                ...
                                <div class="shrink-0 ml-2">
                                    @if (in_array((string) $u->getKey(), $following ?? []))
                                        <form method="POST" action="{{ route('web.friends.unfollow', $u->getKey()) }}">
                                            @csrf
                                            <button class="px-5 py-2.5 border-2 border-[#EAE1FF] bg-slate-50 text-slate-500 rounded-[1rem] text-sm font-bold hover:border-red-200 hover:bg-red-50 hover:text-red-500 transition-all">
                                                Following
                                            </button>
                                        </form>
                                    @else
                                        <form method="POST" action="{{ route('web.friends.follow', $u->getKey()) }}">
                                            @csrf
                                            <button class="px-5 py-2.5 bg-gradient-to-r from-[#B28CFF] to-[#9261F3] text-white rounded-[1rem] text-sm font-bold shadow-md shadow-[#9261F3]/20 hover:shadow-lg hover:-translate-y-0.5 transition-all">
                                                Follow
                                            </button>
                                        </form>
                                    @endif
        ...
        @else
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                ...
                {{-- Following Card --}}
                <div class="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#EAE1FF] overflow-hidden flex flex-col">
                    <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                        <h3 class="font-black text-slate-800 text-xl tracking-tight">Following</h3>
                        <span class="bg-[#F3EFFF] text-[#9261F3] px-3 py-1 rounded-full text-xs font-bold">{{ $followingUsers->count() }}</span>
                    </div>

                    @if ($followingUsers->isEmpty())
                        <div class="p-12 flex-1 flex flex-col items-center justify-center text-center">
                            <div class="w-16 h-16 bg-[#F3EFFF] rounded-[1.5rem] flex items-center justify-center mb-4 rotate-3">
                    ...
                    @else
                        <div class="divide-y divide-slate-50 flex-1 overflow-y-auto max-h-[500px] custom-scrollbar">
                            @foreach ($followingUsers as $u)
                                <div class="flex items-center gap-4 px-6 py-4 hover:bg-[#F3EFFF]/30 transition-colors group">
                                    <div class="w-11 h-11 rounded-[1rem] bg-gradient-to-br from-[#C7AFFF] to-[#B28CFF] flex items-center justify-center text-white font-black text-sm shadow-inner shrink-0">
                                        @if ($u->avatar && str_starts_with($u->avatar, 'http'))
                                            <img src="{{ $u->avatar }}" class="w-full h-full rounded-[1rem] object-cover">
                                        @else
                                            {{ strtoupper(substr($u->name, 0, 1)) }}
                                    ...
                                    <form method="POST" action="{{ route('web.friends.unfollow', $u->getKey()) }}">
                                        @csrf
                                        <button class="w-9 h-9 flex items-center justify-center rounded-full border border-slate-200 text-slate-400 hover:text-red-500 hover:border-red-200 hover:bg-red-50 transition-all opacity-100 lg:opacity-0 group-hover:opacity-100" title="Unfollow">
                ...
                <div class="bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-[#EAE1FF] overflow-hidden flex flex-col">
                    <div class="p-6 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                        ...
                        <span class="bg-[#F3EFFF] text-[#9261F3] px-3 py-1 rounded-full text-xs font-bold">{{ $followerUsers->count() }}</span>
                    </div>
                    ...
                    @else
                        <div class="divide-y divide-slate-50 flex-1 overflow-y-auto max-h-[500px] custom-scrollbar">
                            @foreach ($followerUsers as $u)
                                <div class="flex items-center gap-4 px-6 py-4 hover:bg-[#F3EFFF]/30 transition-colors">
                                    <div class="w-11 h-11 rounded-[1rem] bg-slate-100 border border-slate-200 flex items-center justify-center text-[#9261F3] font-black text-sm shrink-0">
                                        @if ($u->avatar && str_starts_with($u->avatar, 'http'))
                                            <img src="{{ $u->avatar }}" class="w-full h-full rounded-[1rem] object-cover">
                                        @else
                                            {{ strtoupper(substr($u->name, 0, 1)) }}
                                    ...
                                    <div class="shrink-0 ml-2">
                                        @if (!in_array((string) $u->getKey(), $following ?? []))
                                            <form method="POST" action="{{ route('web.friends.follow', $u->getKey()) }}">
                                                @csrf
                                                <button class="text-xs bg-[#F3EFFF] text-[#9261F3] px-4 py-2 rounded-[0.8rem] font-bold hover:bg-[#9261F3] hover:text-white transition-all shadow-sm">
                                                    Follow back
                                                </button>
                                            </form>
                                        @else
                                            <span class="bg-green-50 text-green-500 border border-green-200 px-3 py-1.5 rounded-[0.8rem] text-xs font-bold flex items-center gap-1.5">
