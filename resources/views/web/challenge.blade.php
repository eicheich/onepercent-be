...
@section('content')
    <div class="space-y-6 fade-in pb-10">
        ...
        <div
            ...
            <div class="absolute top-4 right-20 text-3xl opacity-60 drop-shadow-md"><img src="/img/image_home_banner.png" alt="home"></div>
            <div class="absolute bottom-4 left-6 text-xl opacity-40">✦</div>

            <div class="flex items-center gap-4 relative z-10">
                <div
                    class="w-16 h-16 bg-white/20 rounded-[1.5rem] flex items-center justify-center text-3xl backdrop-blur-md border border-white/30 shadow-inner rotate-3 hover:rotate-6 transition-transform">
                    🏆
                </div>
                <div>
                    ...
                    <div class="relative z-10">
                        ...
                        @else
                            <div class="text-center py-12">
                                ...
                                <h3 class="text-2xl font-black text-slate-800 mb-2 tracking-tight">Complete today's task
                                </h3>
                                <p class="text-slate-500 text-sm font-medium mb-8">and earn points to keep your streak
                                    alive.</p>

                                {{-- Upload Proof section --}}
                                @if ($todayChallenge?->is_completed)
                                    <div
                                        class="bg-white rounded-[2rem] shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-slate-100 p-6 md:p-8 mt-6 text-left">
                                        <h3 class="font-extrabold text-slate-800 mb-5 text-lg flex items-center gap-2">
                                            <span class="text-2xl">📤</span> Upload Proof <span
                                                class="text-slate-400 text-sm font-medium ml-1">(Optional)</span>
                                        </h3>

                                        @if (session('proof_result'))
                                            <div
                                                class="bg-gradient-to-br from-[#F3EFFF] to-[#EAE1FF] border border-[#B28CFF]/20 rounded-2xl p-5 mb-5 shadow-sm">
                                                <div class="flex items-center gap-4">
                                                    <div
                                                        class="w-16 h-16 bg-white rounded-full flex items-center justify-center text-2xl font-black text-[#9261F3] shadow-sm">
                                                        {{ session('proof_result.score') }}
                                                    </div>
                                                    <div class="flex-1">
                                                        <p class="font-black text-slate-800 tracking-tight">AI Score &
                                                            Feedback</p>
                                                        <p class="text-sm text-slate-600 mt-1 font-medium leading-relaxed">
                                                            {{ session('proof_result.feedback') }}
                                                        </p>
                                                    </div>
                                                    <div class="hidden md:block text-4xl opacity-50 drop-shadow-sm">🤖</div>
                                                </div>
                                            </div>
                                        @endif
                                        ...
                                        @if ($errors->has('error'))
                                            <div
                                                class="bg-red-50 border border-red-100 text-red-600 rounded-2xl p-4 mb-5 text-sm font-bold flex items-center gap-2">
                                        ...
                                        <form method="POST" action="{{ route('web.challenge.upload') }}"
                                            ...
                                            <div class="border-2 border-dashed border-slate-200 bg-slate-50 rounded-[1.5rem] p-8 text-center hover:border-[#B28CFF] hover:bg-[#F3EFFF]/50 transition-colors cursor-pointer mb-5 group"
                                                onclick="document.getElementById('proofFile').click()">
                                                <div
                                                    class="w-16 h-16 bg-white rounded-full flex items-center justify-center text-3xl mx-auto mb-4 shadow-sm group-hover:scale-110 transition-transform">
                                                    📎
                                                </div>
                                                ...
                                                <p class="text-xs text-slate-400 mt-2 font-medium uppercase tracking-wider">
                                                    PDF, Image, or any file (max 50MB)</p>
                                                <p id="selectedFileName"
                                                    class="text-sm text-[#9261F3] mt-3 font-bold hidden bg-white inline-block px-4 py-1.5 rounded-full border border-[#EAE1FF]">
                                                </p>
                                            </div>
                                            <input type="file" id="proofFile" name="file" class="hidden"
                                                onchange="document.getElementById('selectedFileName').textContent = this.files[0]?.name; document.getElementById('selectedFileName').classList.remove('hidden')">

                                            <button type="submit"
                                                class="w-full bg-[#B28CFF] text-white py-4 rounded-full font-extrabold text-base hover:bg-[#9261F3] hover:shadow-lg hover:-translate-y-0.5 active:scale-95 transition-all">
                                                Upload & Get AI Score ✨
                                            </button>
                                        </form>
                                    </div>
                                ...
                                <button onclick="generateChallenge()"
                                    class="w-full md:w-max mx-auto px-10 inline-flex justify-center items-center gap-2 bg-slate-800 text-white py-4 rounded-full font-extrabold text-base hover:bg-slate-700 hover:shadow-lg hover:-translate-y-0.5 active:scale-95 transition-all mt-6">
                                    Generate Task ✨
                                </button>
